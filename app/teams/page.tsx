...
export default function TeamsPage() {
  ...
  const [tab, setTab] = useState<"scores" | "roster">("scores");
  ...
  return (
    <div className="space-y-4">
      <PageHeader
        title="Teams"
        right={contest ? (
          <Confirm label="End Season & Reset" confirmLabel="End season" onConfirm={archive} className="btn btn-ghost" title="End season and reset" />
        ) : (
      ...
      <div className="grid grid-cols-2 gap-2">
        <button onClick={() => setTab("scores")} className={tab === "scores" ? "btn btn-primary" : "btn btn-ghost"}>Scores</button>
        <button onClick={() => setTab("roster")} className={tab === "roster" ? "btn btn-primary" : "btn btn-ghost"}>Roster</button>
      </div>
              ...
              {unassigned.map((s) => (
                <li key={s.id} className="flex justify-between items-center gap-2">
                  ...
                  <div className="flex gap-1 flex-wrap justify-end">
                    ...
                    <Confirm label="🗑" title="Remove student" onConfirm={() => del(s.id)} className="btn btn-ghost text-xs" />
                  </div>
                </li>
              ))}
            </ul>
      ...
      <div className="grid grid-cols-2 gap-3">
        {teams.map((t) => {
          const roster = students.filter((s) => s.team_id === t.id);
          return (
            <div key={t.id} className="card p-3">
              ...
              <ul className="mt-2 text-sm space-y-1">
                ...
                {roster.length === 0 && <li className="text-xs text-[#9fb0d3]">No students yet</li>}
              </ul>
            </div>
          );
        })}
...
function NewSeasonModal({ teams, onClose, onCreated }: { teams: Team[]; onClose: () => void; onCreated: () => void }) {
  ...
  const create = async () => {
    ...
    setErr(null);
    if (teams.length === 0) { setErr("No team rows exist to assign mascots to."); return; }
    setSaving(true);
