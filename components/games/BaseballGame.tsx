...
export default function BaseballGame({ itineraryId, backHref, backLabel = "← All games" }: { itineraryId: string | null; backHref: string; backLabel?: string }) {
  ...
  const recordGame = async () => {
    ...
    await supabase.from("game_results").insert({
      itinerary_id: itineraryId, game_type: "bible_baseball",
    ...
    for (let i = 0; i < 2; i++) {
      ...
      if (runs > 0) {
        await supabase.from("score_events").insert({ team_id: team.id, itinerary_id: itineraryId, points: runs, reason: "Bible Baseball runs" });
      }
      if (winBonus > 0) {
        await supabase.from("score_events").insert({ team_id: team.id, itinerary_id: itineraryId, points: winBonus, reason: "Bible Baseball win" });
      }
    }
    alert("Game recorded.");
  };
      ...
      {gameOver ? (
        <div className="card p-6 text-center space-y-3">
          ...
          <div className="flex gap-2 justify-center flex-wrap">
            ...
            {itineraryId && <Link href={`/itineraries/${itineraryId}/summary`} className="btn btn-ghost">To Summary</Link>}
          </div>
        </div>
      ) : !current ? (
