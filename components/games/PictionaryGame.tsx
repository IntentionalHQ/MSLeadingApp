...
export default function PictionaryGame({ itineraryId, backHref, backLabel = "← All games" }: { itineraryId: string | null; backHref: string; backLabel?: string }) {
  ...
  const saveResults = async () => {
    ...
    for (let i = 0; i < 2; i++) {
      ...
      if (points[i] > 0) {
        await supabase.from("score_events").insert({ team_id: team.id, itinerary_id: itineraryId, points: points[i], reason: "Bible Pictionary" });
      }
    }
    await supabase.from("game_results").insert({
      itinerary_id: itineraryId, game_type: "bible_pictionary",
