...
export default function HangmanGame({ itineraryId, backHref, backLabel = "← All games" }: { itineraryId: string | null; backHref: string; backLabel?: string }) {
  ...
  const saveResults = async () => {
    ...
    for (let i = 0; i < 2; i++) {
      ...
      if (points[i] > 0) {
        await supabase.from("score_events").insert({ team_id: team.id, itinerary_id: itineraryId, points: points[i], reason: "Bible Hangman" });
      }
    }
    await supabase.from("game_results").insert({
      itinerary_id: itineraryId, game_type: "bible_hangman",
              ...
              {ALPHABET.map((L) => {
                ...
                return (
                  <button
                    ...
                    onClick={() => guess(L)}
                    className={
                      "py-3 rounded text-sm font-bold " +
                      (isGuessed
