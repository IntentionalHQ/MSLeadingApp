...
export default function AuctionGame({ itineraryId, backHref, backLabel = "← All games" }: { itineraryId: string | null; backHref: string; backLabel?: string }) {
  ...
  const saveResults = async () => {
    ...
    if (winnerIdx !== null) {
      ...
      await supabase.from("score_events").insert({ team_id: winner.id, itinerary_id: itineraryId, points: 1, reason: "Bible Auction win" });
    }
    await supabase.from("game_results").insert({
      itinerary_id: itineraryId, game_type: "bible_auction",
